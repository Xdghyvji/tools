...
export async function getUserStatus(user) {
    ...
    try {
        ...
        if (snapshot.exists()) {
            ...
            return { isVip, plan: data.plan || 'free', name: data.name || user.displayName };
        }
    } catch (error) { console.error("Error fetching user status:", error); }
    return { isVip: false, plan: 'free' };
}

// ===============================================================
// 6. UI COMPONENTS (Header, Footer & Cookies)
// ===============================================================
...
function initAuthUI() {
    onAuthStateChanged(auth, async (user) => {
        ...
        if (user) {
            ...
            if(desktopArea) {
                desktopArea.innerHTML = `
                    <div class="relative group cursor-pointer">
                        <div class="w-9 h-9 rounded-full bg-slate-900 text-white flex items-center justify-center font-bold border-2 border-slate-100 shadow-sm">${initial}</div>
                        ${vipBadge}
                        <div class="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-2 hidden group-hover:block animate-fade-in z-50">
                            <div class="px-4 py-2 border-b border-slate-50 mb-1">
                                ...
                                <p class="font-bold text-sm truncate text-slate-900">${name || 'User'}</p>
            ...
            if(mobileArea) {
                mobileArea.innerHTML = `
                    <div class="flex items-center gap-3 border-t border-slate-100 pt-4 mt-2">
                        ...
                        <div><p class="text-sm font-bold text-slate-900">${name || 'User'}</p>${isVip ? '<p class="text-xs text-yellow-600 font-bold">VIP Member</p>' : '<p class="text-xs text-slate-500">Free Plan</p>'}</div>
            ...
            if (window.lucide) window.lucide.createIcons();
            if (window.location.pathname.includes('subscription.html') && isVip) markVipPlanActive();
...
function handleLogout(e) { e.preventDefault(); signOut(auth).then(() => window.location.href = '/index.html'); }
    ...
    footer.innerHTML = `
        <div class="bg-slate-900 text-white py-12 border-t border-slate-800">
            ...
            <div class="max-w-7xl mx-auto px-6 mt-12 pt-8 border-t border-slate-800 text-center text-slate-500 text-sm">
                &copy; ${year} Digital Services Hub.
            </div>
        </div>`;
    
    // *** TRIGGER COOKIE POPUP HERE ***
    initCookieConsent();
    if (window.lucide) window.lucide.createIcons();
}

// ===============================================================
// 7. COOKIE CONSENT LOGIC (AdSense Compliant)
// ===============================================================
function initCookieConsent() {
    ...
    document.getElementById('cookie-reject').addEventListener('click', () => {
        // We still save 'true' to stop showing the banner, 
        // but in a real GDPR setup, you would disable ad scripts here.
        // For simple AdSense, this just closes the UI.
        localStorage.setItem('dsh_cookie_consent', 'true'); 
        banner.remove();
    });
}
